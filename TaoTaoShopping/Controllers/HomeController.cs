...
namespace TaoTaoShopping.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Pay2(int shopId,int shopNum)
        {
            ...
            if (id == 0)
            {
                return Content("<script>alert('Please login before purchasing an item！');window.location.href='/Home/Login';</script>");
            }
            //查询购物车列表返回
            shopping s = db.shopping.FirstOrDefault(p => p.id == shopId);
            if (s.number < shopNum)
            {
                return Content("<script>alert('" + s.title + "Item out of stock！');window.history.back(-1);</script>");
            }
            //商品数量
        ...
        public ActionResult Pay2(int address_id, string payWay, string mark, int shopId, int shopNum)
        {
            ...
            {
                uid = id,
                order_num = "o" + DateTime.Now.ToString("yyMMddhh"),
                sum_price = sum,
                mark = mark,
        ...
        public ActionResult Pay()
        {
            ...
            foreach(shop_car item in list)
            {
                ...
                {
                    return Content("<script>alert('"+s.title+ "Item out of stock，Please remove the item from the shopping cart first！');window.history.back(-1);</script>");
                }
            }
        ...
        public ActionResult Register(user u)
        {
            ...
            db.user.Add(u);
            db.SaveChanges();
            return Content("<script>alert('Registration Successful！');window.history.back(-1);</script>");
        }
    }
}